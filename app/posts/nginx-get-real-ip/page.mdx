...
- 明明设置了 `allow ip` 和 `deny ip` 却还不生效
- 日志里出现大量从未连接过的 IP 地址
- 每个层级的 Nginx 都要单独处理，容易遗漏
...
如果你对这篇文章感兴趣，不妨去看看站里的另一篇文章[通用 NGINX OIDC 认证集成指南](https://blog.feegr.cc/posts/nginx-oidc)，它介绍了如何利用中间件让 Nginx 支持 OIDC 验证对接 Idp 作为小型的零信任验证网关。
